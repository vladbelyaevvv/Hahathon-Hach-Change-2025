...
4. **Обновите docker-compose.yml** (при необходимости):
   - Измените маппинг порта frontend с `80:80` на `3000:80`, если порт 80 занят
   - Обновите переменные окружения при необходимости
...
6. **Доступ к приложению:**
   ...
   - Backend: `http://your-vps-ip:8080/api`
...
- **[Методичка по бэкенду](BACKEND_GUIDE_RU.md)** - Подробное описание архитектуры и работы backend
- **[Быстрый старт на Windows](QUICKSTART_WINDOWS_RU.md)** - Инструкция по запуску на Windows
- **[Быстрый старт на Linux](QUICKSTART_RU.md)** - Инструкция по запуску на Linux/Ubuntu

## Примечания для разработки

- Это hackathon MVP - код упрощен и не готов для продакшена
- Аутентификация использует простое сравнение паролей (не хешированные)
- Загруженные файлы хранятся локально в директории `uploads`
- ID студента сейчас захардкожен как `1` во frontend (можно улучшить с правильной аутентификацией)

## Решение проблем

### Backend не запускается
- Проверьте, что PostgreSQL запущен и доступен
- Убедитесь в правильности учетных данных БД в `application.yml`
- Проверьте логи: `docker-compose logs backend`

### Frontend не может подключиться к backend
- Убедитесь, что backend запущен на порту 8080
- Проверьте конфигурацию CORS
- В Docker убедитесь, что сервисы находятся в одной сети

### Ошибки подключения к базе данных
- Убедитесь, что контейнер PostgreSQL здоров: `docker-compose ps`
- Проверьте, что учетные данные БД совпадают в `docker-compose.yml` и `application.yml`
- Убедитесь, что тома правильно смонтированы

## Скриншоты

Все скриншоты приложения находятся в папке `screenshots/`:

- `main-page.png` - Главная страница с hero-секцией
- `login-page.png` - Страница входа
- `courses-page.png` - Список курсов
- `course-details.png` - Детали курса
- `assignment-page.png` - Страница задания
- `submissions-history.png` - История отправок
- `swagger-ui.png` - Документация API
